...
void ChArUcoDetector::applyGaussianBlur(cv::Mat &image_in_out_) {
    cv::GaussianBlur(image_in_out_, image_in_out_, cv::Size(5, 5), 0);  // Applying a Gaussian blur
}

// Modify the imageCallback function to use Gaussian blur instead of median blur
    ...
    if (camera_info_) {
        ...
        if ((_msg->encoding == sensor_msgs::image_encodings::MONO16 || use_dynamic_range_)) {
            try {
                ...
                applyGaussianBlur(image_grayscale);  // Apply Gaussian blur instead of median blur
        ...
        if (!dynamic_range_applied) {
            try {
                ...
                applyGaussianBlur(image_grayscale);  // Apply Gaussian blur here too
